...
# %%
data = np.loadtxt('coil_data_2.5v_outhalf.csv', delimiter=',').transpose()
data.shape
# %%
bx = data[2]
by = data[3]
...
plt.errorbar(range(82*3), bx, yerr=sbx, label = "Bx")
plt.errorbar(range(82*3), by, yerr=sby, label = "By")
plt.errorbar(range(82*3), bz, yerr=sbz, label = "Bz")
plt.legend()
plt.show()
# %%
